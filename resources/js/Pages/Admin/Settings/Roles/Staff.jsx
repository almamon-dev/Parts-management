...
import { User, Plus, Search, Mail, Edit3, Trash2 } from "lucide-react";
                ...
                <div className="flex items-center gap-8 border-b border-gray-100 mb-8">
                    <Link
                        ...
                        className={`pb-4 text-sm font-medium transition-colors relative ${
                            route().current("admin.settings.roles.index")
                                ? "text-[#FF9F43]"
                    ...
                    >
                        Roles
                        {route().current("admin.settings.roles.index") && (
                            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#FF9F43]" />
                        )}
                    </Link>
                    <Link
                        href={route("admin.settings.permissions.index")}
                        className={`pb-4 text-sm font-medium transition-colors relative ${
                            route().current("admin.settings.permissions.index")
                                ? "text-[#FF9F43]"
                                : "text-gray-500 hover:text-gray-700"
                        }`}
                    >
                    ...
                    <Link
                        ...
                        className={`pb-4 text-sm font-medium transition-colors relative ${
                            route().current("admin.settings.staff.index")
                                ? "text-[#FF9F43]"
                                : "text-gray-500 hover:text-gray-700"
                        }`}
                    >
            ...
            <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        {user.company_name && (
                            <div className="text-[11px] text-[#FF9F43] font-bold mt-0.5 uppercase tracking-wide">
                                {user.company_name}
                            </div>
                        )}
                    </div>
                ...
                {isEditing ? (
                    ...
                    <div className="space-y-2">
                        <div className="flex flex-wrap gap-1">
                            ...
                            {user.roles?.filter((role) => role.name !== "Admin")
                                .length > 2 && (
                        ...
                        {user.permissions.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                                {user.permissions.map((perm) => (
                                    <span
                                        key={perm.id}
                                        className="px-1.5 py-0.5 bg-orange-50 text-[#e68a2e] rounded text-[9px] font-medium border border-orange-100"
                                    >
                                        {perm.name}
                                    </span>
                                ))}
