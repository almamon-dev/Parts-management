...
export default function Create() {
    ...
    if (
        data.country &&
        data.province &&
        TAX_RATES[data.country] &&
        TAX_RATES[data.country][data.province]
    ) {
        taxRate = TAX_RATES[data.country][data.province].rate;
    } else if (!data.country || !data.province) {
        // Default to Ontario 13% if not specified (consistent with Invoice fallback)
        taxRate = 0.13;
    }

    const discountAmount = parseFloat(data.discount) || 0;
    const netAmount = Math.max(0, totalSell - discountAmount);
